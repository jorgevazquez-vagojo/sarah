...
RDGBot es una plataforma premium de atencion al cliente que combina un **chatbot con inteligencia artificial multi-proveedor**, un **sistema de telefonia VoIP completo** (WebRTC + RDGPhone) y un **dashboard profesional para agentes humanos**, todo integrable como widget embebible en cualquier sitio web.
...
| **VoIP Completo** | Llamadas desde el navegador via WebRTC (Janus Gateway) y RDGPhone con devolucion de llamada via SIP/Vozelia. |
...
| **VoIP RDGPhone** | SIP UDP nativo | RFC 3261 | Llamadas PSTN via Vozelia |
...
### CU-02: Visitante Solicita Callback (RDGPhone)
...
**Precondicion:** Llamada en curso (RDGPhone o WebRTC)
**Flujo principal:**
1. Al iniciar una llamada RDGPhone, se envia header SIP `Record: on` a la PBX.
2. Para llamadas WebRTC, Janus activa grabacion automatica (`record: true`).
3. Las grabaciones se almacenan en `/server/recordings/` (RDGPhone) o `/janus/recordings/` (WebRTC).
...
```bash
git clone https://github.com/jorgevazquez-vagojo/rdgbot.git
cd rdgbot
```
...
| `POSTGRES_DB` | No | `rdgbot` | Nombre de la base de datos |
...
| `calls` | Cientos/mes | Registros de llamadas (RDGPhone + WebRTC) |
...
**Archivo:** `plugins/wordpress/rdgbot.php`

Caracteristicas:
- Panel de configuracion completo en Settings > RDGBot.
...
**Instalacion:**
1. Copiar `rdgbot.php` a `wp-content/plugins/rdgbot/`.
2. Activar en Plugins.
3. Configurar en Settings > RDGBot.

### 14.4 Snippet Shopify

**Archivo:** `plugins/shopify/assets/rdgbot.liquid`

Caracteristicas:
- Snippet Liquid renderizable con `{% render 'rdgbot' %}`.
...
**Instalacion:**
1. Subir `rdgbot.liquid` a la carpeta `assets/` del tema.
2. Importar `settings_schema.json` en la configuracion del tema.
3. Anadir `{% render 'rdgbot' %}` antes de `</body>` en `theme.liquid`.
...
**Instalacion:**
1. Crear modulo en `app/code/Redegal/RdgBot/`.
...
**Opciones configurables en `window.RdgBot`:**
...
docker compose exec postgres pg_dump -U redegal rdgbot > backup.sql

# Restaurar base de datos
docker compose exec -T postgres psql -U redegal rdgbot < backup.sql
```
...
| **RDGPhone** | Llamada de devolucion: el servidor llama al visitante y lo conecta con un agente |
